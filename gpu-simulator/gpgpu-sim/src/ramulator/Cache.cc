...
void Cache::evict(std::list<Line>* lines,
  ...
  if (!is_last_level) {
    ...
    // LLC eviction
    if (dirty) {
      Request write_req(addr, Request::Type::R_WRITE, 0); // write memory requests are caused by eviction at LLC, which is highly reordered and most of them suffer from conflicts. Here we don't differentiate statistics related with write requests from different cores.
