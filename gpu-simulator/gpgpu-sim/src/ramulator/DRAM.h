...
public:
    unsigned m_id;
    ScalarStat memory_partition_id;
    ScalarStat active_cycles;
    ScalarStat dram_copy_evict_active_cycles;
    ...
    // Constructor
    DRAM(T* spec, typename T::Level level,unsigned m_id);
    DRAM(T* spec, typename T::Level level,unsigned m_id, DRAM<T>* parent, unsigned id);
    ~DRAM();
    ...
    // sungjun: for PCM, it checks whether the row buffer is dirty or not
    bool isDirtyRow;
    ...
    //int check_status(typename T::Command cmd, const int* addr, long clk);
    ...
    long begin_of_cur_dram_copy_evict_reqcnt = -1;
    long begin_of_refreshing = -1;
    long end_of_refreshing = -1;

    typename T::Command prevIssuedCmd;
    unsigned long rw_turnaround;
    unsigned long wr_turnaround;

    std::vector<std::pair<long, long>> refresh_intervals;

    // register statistics
    void regStats(const std::string& identifier, int c, typename T::Level level);
...
// register statistics
template <typename T>
void DRAM<T>::regStats(const std::string& identifier, int channel_id, typename T::Level level_cond) {
    if (channel_id == 0 && level_cond == T::Level::Channel) {
      memory_partition_id
        .name("memory_partition_id: ")
        .desc("memory partition id")
        .precision(0)
        ;
      memory_partition_id += m_id;
    }
    active_cycles
        ...
        .precision(0)
        ;
    dram_copy_evict_active_cycles
        .name("dram_copy_evict_active_cycles" + identifier + "_" + to_string(id))
        .desc("Total active cycles for level " + identifier + "_" + to_string(id))
        .precision(0)
        ;
    refresh_cycles
    ...
    for (auto child : children) {
      child->regStats(identifier + "_" + to_string(id), -1, T::Level::MAX);
    }
}

// Constructor
template <typename T>
DRAM<T>::DRAM(T* spec, typename T::Level level, unsigned m_id) :
    spec(spec), level(level), id(0), parent(NULL), m_id(m_id)
{
    isDirtyRow = false;
    ...
    for (int i = 0; i < child_max; i++) {
      DRAM<T>* child = new DRAM<T>(spec, typename T::Level(child_level), m_id);
      child->parent = this;
      child->id = i;
...
template <typename T>
DRAM<T>::DRAM(T* spec, typename T::Level level, unsigned m_id,
              DRAM<T>* parent, unsigned id) :
    spec(spec), level(level), id(id), parent(parent), m_id(m_id)
{
    isDirtyRow = false;
    ...
    for (int i = 0; i < child_max; i++) {
      DRAM<T>* child = new DRAM<T>(spec, typename T::Level(child_level), m_id, this, id);
      //child->parent = this;
      //child->id = i;
      children.push_back(child);
    }
...
// Insert
// TODO: need to implement insert for HMS later
template <typename T>
...
/*
template <typename T>
int DRAM<T>::check_status(typename T::Command cmd, const int* addr, long clk) {
  if (next[int(cmd)] != -1 && clk < next[int(cmd)]) {
    return status[int(cmd)];
  }
  int child_id = addr[int(level)+1];
  if (child_id < 0 || level == spec->scope[int(cmd)] || !children.size())
      return static_cast<int>(T::Status::READY_TO_ISSUE); // stop recursion: the check passed at all level
  // recursively check my child
  return children[child_id]->check_status(cmd, addr, clk);
}
*/
...
// TODO: need to implement for HMS
// However, get_next() does not require for HMS
template <typename T>
...
// This function exists only for HMS
//template <typename T>
//void DRAM<T>::update_timer(typename T::Command cmd, const int* addr, long clk) {}
...
void DRAM<T>::update_timing(typename T::Command cmd, const int* addr, long clk)
{
    ...
    // channel does not execute following if statement
    // Follwoing if statement exists for rank
    ...
    // sungjun: in our system, channel and rank must come here
    for (auto& t : timing[int(cmd)]) {
        if (t.sibling)
            continue; // not an applicable timing parameter
        ...
        if (future > next[int(t.cmd)]) {
          //status[int(t.cmd)] = static_cast<int>(t.status);
          next[int(t.cmd)] = future;
        }
        //next[int(t.cmd)] = max(next[int(t.cmd)], future); // update future
