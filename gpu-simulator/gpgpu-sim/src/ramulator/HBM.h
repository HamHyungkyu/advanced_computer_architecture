...
class HBM {
 public:
  ...
  enum class Status : int {
    ...
    WRITE_TO_PRE,
    BANK_ACT_TO_PRE,
    BANK_ACT_TO_ACT_DELAY,
    BG_ACT_TO_ACT_DELAY,
    RANK_ACT_TO_ACT_DELAY,
    TFAW_ACT_TO_ACT_DELAY,
    READY_TO_ISSUE,
    MAX
  };
  /* Level */
  ...
  enum class Org : int {  // per channel density here. Each stack comes with 8
                          // channels TITAN V has 3 stacks (total 12GBs) each
                          // stack (4GBs) owns four dram dies with 2 channels
                          // each each dram die owns 1GB -> 1GB/2 channels = 4Gb
                          // (0.5 GB) per channel
    HBM_1Gb,
    HBM_2Gb,
  ...
  struct AddrMask {
    string mask;
      ...
      {
          {"00000000.00000000.00000000.00000000.00000RRR.RRRRRRRR.RRBGGCCC."
           "CCCSSSSS"},
          {"00000000.00000000.00000000.00000000.0000RRRR.RRRRRRRR.RRBGGCCC."
           "CCCSSSSS"},
          {"00000000.00000000.00000000.00000000.000RRRRR.RRRRRRRR.RBBGGCCC."
           "CCCSSSSS"},
          {"00000000.00000000.00000000.00000000.0000RRRR.RRRRRRRR.RBGGHCCC."
           "CCCSSSSS"},
          {"00000000.00000000.00000000.00000000.000RRRRR.RRRRRRRR.RBGGHCCC."
           "CCCSSSSS"},
          {"00000000.00000000.00000000.00000000.00RRRRRR.RRRRRRRR.BBGGHCCC."
           "CCCSSSSS"},
  },
    addr_mask;
  ...
  // prefetch
  int prefetch_size =
      2;  // burst length could be 2 and 4 (choose 4 here), 2n prefetch
  ...
  struct SpeedEntry {
    ...
    int nRCD_TR_RD;
      ...
      {{1000, 500, 2.0, 0, 2, 2,  3, 7,    7, 6, 7, 4, 17, 24, 7,
        2,    4,   8,   4, 5, 20, 0, 1950, 0, 5, 5, 5, 0,  0},

       {2000, 1000, 1.0, 0,  1, 1, 2,  14, 14,   14, 14, 2, 33, 47, 3,
        4,    3,    8,   16, 4, 6, 16, 0,  3900, 0,  0,  0, 0,  0,  0},

       // down-scaled from HBM_2Gbps
       {1752, 876, 1.1415, 0,  1, 1, 2,  12, 12,   12, 12, 2, 29, 41, 3,
        4,    3,   7,      14, 4, 5, 14, 0,  3417, 0,  0,  0, 0,  0,  0}

  },
    speed_entry;

  int read_latency;
  // sungjun: add write latency
  int write_latency;
