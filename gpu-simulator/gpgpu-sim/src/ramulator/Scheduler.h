...
public:
    ...
    // Make scheduler type configurable when instantiate scheduler object
    Scheduler(Controller<T>* ctrl, Type schedulerType = Type::FRFCFS_PriorHit) : ctrl(ctrl) {
      std::cout << "Normal Scheduler is instantiated" << std::endl;
      type = schedulerType;
    }

    void setSchedulerType(Type schedulerType) {
      type = schedulerType;
    }
    ...
    list<Request>::iterator get_head(list<Request>& q)
    {
      ...
      if (type != Type::FRFCFS_PriorHit) {
        ...
        // only returns when the command of the head is READ, WRITE, COPY, or EVICT
...
private:
    ...
    function<ReqIter(ReqIter, ReqIter)> compare[int(Type::MAX)] = {
        ...
        [this] (ReqIter req1, ReqIter req2) {
            ...
            // if one of requests is ready to be scheduled
            if (ready1 ^ ready2) {
                if (ready1) return req1;
                return req2;
            }
            ...
            //printf("request1 arrive: %d, request2 arrive: %d\n", req1->arrive, req2->arrive);

            //assert(false && "queue scheduling error");
            return req2;
        }
    };
};
